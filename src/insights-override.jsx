...
function buildPersonalInsights(state){
  ...
  if (entries.length === 0) {
    cards.push({
      ...
      title: 'No data yet',
      value: 'Start logging',
      body: 'Log your first weigh-in. After a few days, this page will start showing personalized insights from your own trend.',
  ...
  cards.push({
    ...
    body: avgDelta7 == null
      ? 'You need a few more logged days before the app can compare your current 7-day average to last week.'
      : Math.abs(avgDelta7) < 0.2
        ? `Your 7-day average is basically flat compared with a week ago. That usually means your trend is stable, not broken.`
        : `Your 7-day average is ${avgDelta7 > 0 ? 'up' : 'down'} ${Math.abs(avgDelta7).toFixed(1)} ${unit} compared with a week ago. This is more useful than judging one morning.`
  });
  ...
  cards.push({
    ...
    body: thisWeek.length >= 5
      ? 'Great consistency. A week with 5+ logs gives your averages way more trust than random single weigh-ins.'
      : thisWeek.length >= 3
        ? 'Decent start. A few more weigh-ins this week will make the average more reliable.'
        : 'Low logging this week. The trend can still help, but confidence is lower until you log more days.'
  });
  ...
  cards.push({
    ...
    body: fluctuation == null
      ? 'After more entries, WeightLens will estimate your normal day-to-day scale noise.'
      : `Your usual day-to-day movement is about ±${fluctuation.toFixed(1)} ${unit}. Spikes inside that range are usually normal water, food, sodium, sleep, or training noise.`
  });
  ...
  cards.push({
    ...
    body: streak >= 7
      ? 'Strong streak. This is exactly how you make the trend trustworthy.'
      : streak >= 3
        ? 'Nice momentum. Keep it boring and consistent.'
        : 'No stress — just log tomorrow morning. The goal is consistency, not perfection.'
  });
  ...
  if (weekDelta != null) {
    cards.push({
      ...
      body: Math.abs(weekDelta) < 0.3
        ? 'This week is almost identical to last week. That is a stable trend, not automatically a plateau.'
        : `This week’s average is ${weekDelta > 0 ? 'higher' : 'lower'} than last week by ${Math.abs(weekDelta).toFixed(1)} ${unit}. Use this with your goal, not in isolation.`
    });
  }
  ...
  if (rate != null) {
    ...
    if (goal.mode === 'fatloss') {
      if (rate < -0.3 && rate > -1.5) {
        goalTone = 'good';
        goalBody = `Your trend is moving down at a reasonable fat-loss pace: ${Math.abs(rate).toFixed(2)} ${unit}/week.`;
      } else if (rate <= -1.5) {
        goalTone = 'warn';
        goalBody = `You are trending down fast: ${Math.abs(rate).toFixed(2)} ${unit}/week. Watch recovery, hunger, and gym performance.`;
      } else if (rate > 0.3) {
        goalTone = 'warn';
        goalBody = `Your trend is moving up even though your goal is fat loss. Look at the last 2 weeks before changing anything drastic.`;
      }
    }

    if (goal.mode === 'musclegain') {
      if (rate > 0.15 && rate < 1.0) {
        goalTone = 'good';
        goalBody = `Your trend is moving up slowly: ${rate.toFixed(2)} ${unit}/week. That is usually better for lean gaining than rushing.`;
      } else if (rate >= 1.0) {
        goalTone = 'warn';
        goalBody = `Your gain rate is pretty fast: ${rate.toFixed(2)} ${unit}/week. Some of that may be water or fat.`;
      }
    }
    ...
    cards.push({
      ...
      value: rate >= 0 ? `+${rate.toFixed(2)}` : rate.toFixed(2),
      body: goalBody
    });
  }
  ...
  if (weekend && Math.abs(weekend.diff) >= 0.4) {
    cards.push({
      ...
      body: weekend.diff > 0
        ? `Your weekend average has been about ${weekend.diff.toFixed(1)} ${unit} higher than weekdays recently. That can happen from restaurant meals, later eating, sodium, or changed routine.`
        : `Your weekend average has actually been lower than weekdays recently. Your routine may be tighter on weekends, or this may just be normal data noise.`
    });
  }
  ...
  cards.push({
    ...
    body: logPercent >= 0.75
      ? 'Your recent data quality is strong. The app can trust your averages more.'
      : logPercent >= 0.5
        ? 'Your data is usable, but a few more logs would make insights sharper.'
        : 'Your recent data is a little thin. Log more often before making big decisions from the trend.'
  });
...
function InsightToneIcon({ tone }){
  ...
  return (
    <div className={`h-9 w-9 shrink-0 rounded-xl flex items-center justify-center ${cls}`}>
      {tone === 'good' ? <I.Check className="h-4 w-4"/> : tone === 'warn' ? <I.Alert className="h-4 w-4"/> : <I.Sparkle className="h-4 w-4"/>}
    </div>
  );
}
    ...
    <Card className="h-full">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {card.action && (
            <Button className="mt-4" variant="secondary" onClick={onAction}>{card.action}</Button>
          )}
        </div>
      </div>
    </Card>
...
function InsightsPage(){
  ...
  return (
    <div className="space-y-6 fadein">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-mute mb-1.5">Personalized</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {personalized.map((card, i) => (
          <PersonalizedInsightCard key={card.title + i} card={card} onAction={()=>card.route && setRoute(card.route)}/>
        ))}
      </div>
...
Object.assign(window, { InsightsPage, buildPersonalInsights });
